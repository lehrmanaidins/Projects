...
'''
    Updated Line Detect Script

    This is the updated line detection script 
    that draws a minimum area rotated rectangle
    instead of a bounding box.

    File: updated_line_detect_script.py
    Author: Original Author + Aidin Lehrman
    Version: 02-27-2024 06:41
'''
...
def process_frame_for_line(frame, image_format="PIL", process_image_width=320, crop_ratio = 4):

    cv2 = import_opencv()

    # (left, upper, right, lower)-tuple
    cropped_frame = ImageFunctions.convert(frame, format="PIL").crop((0, frame.height / crop_ratio - 1, frame.width - 1, frame.height - 1))
    cv_frame = ImageFunctions.convert(cropped_frame, format="OpenCV")
    ...
    if line_contour is not None:
        ...
        bounding_rectangle = cv2.minAreaRect(line_contour)
        rectangle_dimensions = cv2.boxPoints(bounding_rectangle)
